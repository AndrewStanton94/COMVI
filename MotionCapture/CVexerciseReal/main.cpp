...
SiftFeatureDetector detector(200);// one of parameters could be changed and improved //---dictionary size=number of cluster's centroids
int clusterSize = 1000; // one of parameters could be changed and improved for further use
...
// in case to handle many videos for training
int CountLines(char *filename)
{
	ifstream ReadFile;
	int n = 0;
	char line[512];// you can improve the method 
	string temp;
	ReadFile.open(filename, ios::in);//ios::in reading method 
	if (ReadFile.fail())//failed reading return 0
	{
		return 0;
	}
	else// the file 
...
int main(int argc, char** argv)
{
	...
	VideoCapture cap = 0;
	fstream in("AllFiles.txt");
	int LineNumbers;
	LineNumbers = CountLines("AllFiles.txt");
	for (int fnum = 1; fnum <= LineNumbers; fnum++)
	{
		...
		fname = "D:\\Port\\COMVI\\DB\\walking\\person01_walking_d1_uncomp.avi";//change the path according to your own dataset
		...
		imshow("MHI", SAMHI_10); // Try to uncomment these imshow and compare the differences
